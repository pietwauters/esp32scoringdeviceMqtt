...
cJSON *cyrano_to_json(const std::string &cyrano_str) {
  ...
  if (!header.empty() && header.front().empty())
  ...
  if (groups.size() == 3) {
    ...
    if (!right.empty() && right.back().empty())
      right.pop_back();
    right.resize(12, "");
    for (int i = 0; i < 12; ++i) {
      if (right[i] != "") {
...
std::string json_to_cyrano(cJSON *json) {
  ...
  if (true) {
    ...
    for (int i = 0; i < 12; ++i) {
      cJSON *item =
          cJSON_GetObjectItemCaseSensitive(json, right_fencer_fields[i]);
    ...
    if (right.empty())
      return oss.str();
    ; ///// we're done, there are no fencers parts
    oss << "|" << join(right, '|') << "|%";
    ...
    for (int i = 0; i < 12; ++i) {
      cJSON *item =
          cJSON_GetObjectItemCaseSensitive(json, left_fencer_fields[i]);
...
// Create a UW2F_Timer JSON object. `time_ms` is the timer value in ms,
// `timestamp_ms` is epoch milliseconds when the timer event was generated.
cJSON *make_uw2f_timer(const std::string &piste, int time_ms,
                       long long timestamp_ms) {
  cJSON *root = cJSON_CreateObject();
  if (!root)
    return nullptr;

  cJSON_AddStringToObject(root, "Protocol", "EFP2");
  cJSON_AddStringToObject(root, "Piste", piste.c_str());
  ...
  cJSON_AddNumberToObject(timer, "time_ms", time_ms);
  // Use a numeric field for epoch ms; cJSON stores numbers as double.
  cJSON_AddNumberToObject(timer, "timestamp_ms", (double)timestamp_ms);

  cJSON_AddItemToObject(root, "UW2F_Timer", timer);

  return root;
}

std::string make_uw2f_timer_string(const std::string &piste, int time_ms,
                                   long long timestamp_ms) {
  cJSON *json = make_uw2f_timer(piste, time_ms, timestamp_ms);
  if (!json)
    return "";
...
// eventtype contains a 24-bit time value: (minutes<<16) | (seconds<<8) |
// hundredths
cJSON *make_uw2f_timer_from_event(uint32_t eventtype, const std::string &piste,
                                  long long timestamp_ms) {
  uint32_t data = eventtype & 0x00ffffffu;
  uint8_t hundredths = data & 0xffu;
  uint8_t seconds = (data >> 8) & 0xffu;
  uint8_t minutes = (data >> 16) & 0xffu;

  char timestr[16];
  // Format minutes:seconds. Ignore hundredths for this representation.
  std::snprintf(timestr, sizeof(timestr), "%u:%02u", (unsigned)minutes,
                (unsigned)seconds);
  ...
  cJSON_AddStringToObject(timer, "time", timestr);
  cJSON_AddNumberToObject(timer, "timestamp_ms", (double)timestamp_ms);
...
std::string make_uw2f_timer_from_event_string(uint32_t eventtype,
                                              const std::string &piste,
                                              long long timestamp_ms) {
  cJSON *json = make_uw2f_timer_from_event(eventtype, piste, timestamp_ms);
  if (!json)
    return "";
  char *s = cJSON_PrintUnformatted(json);
  std::string result = s ? std::string(s) : std::string("");

...
typedef std::function<void(const char *topic, const char *payload, size_t len)>
    mqtt_message_cb_t;
...
public:
  ...
  void publish(const char *topic, int qos, bool retain, const void *payload,
               size_t length);

  // Optional: for safer string handling
  void publishString(const char *topic, int qos, bool retain,
  ...
  void setWill(const char *topic, const char *message, int qos = 0,
               bool retain = false);
  void clearWill();

  void setTlsCerts(const char *cert_pem, const char *key_pem = nullptr,

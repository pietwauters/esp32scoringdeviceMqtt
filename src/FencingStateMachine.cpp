...
FencingStateMachine::FencingStateMachine(int hw_timer_nr, int tickPeriod) {
  ...
  // Semaphore in Counting mode to allow some jitter if some tasks take a bit
  // longer than the FSMPeriod
  ...
  //  (with a different scaler?) Set 8 divider for prescaler (see ESP32
  //  Technical Reference Manual for more info).
...
void FencingStateMachine::update(MultiWeaponSensor *subject,
                                 uint32_t eventtype) {
  // SetMachineLights(subject->get_Lights());
  uint32_t maineventtype = eventtype & MAIN_TYPE_MASK;
  if (EVENT_LIGHTS == maineventtype)
    SetMachineLights(
        eventtype); // I'm doing an implicit type cast from uint32_t to uint8_t
...
void FencingStateMachine::TransmitFullStateToDisplay(
    RepeaterSender *TheRepeater) {

  // StateChanged(EVENT_LIGHTS | m_Lights);   // Wrong, this will cause
  // everything to transmit
  ...
  // TheRepeater->update(this,EVENT_UW2F_TIMER | (m_UW2FSeconds/60)<<16 |
  // (m_UW2FSeconds%60)<<8);
}
...
void FencingStateMachine::update(UDPIOHandler *subject, uint32_t eventtype) {
  ...
  if (m_Timer.IsRunning()) {
    ...
    if (UI_INPUT_STOP_TIMER !=
        event_data) // Ignore all buttons except stop timer
      return;
  } else {
    if (UI_INPUT_TOGGLE_TIMER == event_data) {
  ...
  case UI_INPUT_START_TIMER:
    if (m_Timerstate != MATCH_ENDED) {
      ...
      if ((FIGHTING == m_Timerstate) &&
          (m_nrOfRounds > 1)) // We only deal with unwillingness to fight in
                              // direct eleminiation and team events
      {
        if (m_Timer.GetMinutes() * 60 + m_Timer.GetSeconds() >
            60) // don't start a new period if there is less than a minute left
        {
          m_UW2FTimer.Start();
        } else {
          if (m_UW2FTimer.GetIntermediateTime() >
              0) // The timer had already been running without reset, so
                 // continue
          {
            m_UW2FTimer.Start();
          }
  ...
  case UI_INPUT_RESTORE_UW2F_TIMER:
    ...
    StateChanged(EVENT_UW2F_TIMER | (m_UW2FSeconds / 60) << 16 |
                 (m_UW2FSeconds % 60) << 8);
...
void FencingStateMachine::ProcessUW2FUndo() {
  ...
  if (m_PCardLeft == 1) { // in this case also m_PCardRight = 1 -> just remove
                          // the cards, no score change needed
    m_PCardLeft = 0;
    m_PCardRight = 0;
    m_UW2FTimer.RestorePreviousState();
    m_UW2FSeconds = m_UW2FTimer.GetIntermediateTime();
  ...
  if ((m_PCardLeft == 2) &&
      (m_PCardRight == 2)) { //-> show yellow cards and change the score
    m_PCardLeft = 1;
    m_PCardRight = 1;
    m_ScoreRight--;
    m_ScoreLeft--;
  ...
  if ((m_PCardLeft == 4) &&
      (m_PCardRight == 4)) { //-> show yellow cards and change the score
    m_PCardLeft = 2;
    m_PCardRight = 2;
...
uint32_t FencingStateMachine::MakeTimerEvent() {
  uint32_t temp = (m_Timer.GetMinutes() << 16 | m_Timer.GetSeconds() << 8 |
                   m_Timer.GetHundredths());
...
void FencingStateMachine::DoStateMachineTick() {
  bool idle = true;
  // while (!xSemaphoreTake(timerSemaphore_FSMPeriod, 1 / (portTICK_PERIOD_MS))
  // == pdTRUE); // check with 1 ms timeout
...
bool FencingStateMachine::incrementScoreAndCheckForMinuteBreak(
    bool bLeftFencer) {
...
void FencingStateMachine::PeriodicallyBroadcastFullState(
    class RepeaterSender *TheRepeater, long Period) {

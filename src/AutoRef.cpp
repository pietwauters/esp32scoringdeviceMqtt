...
void AutoRef::AutoRefHandler(void *parameter) {
  ...
  while (true) {
    if (xQueueReceive(MyLocalAutoRef.AutoRefqueue, &LastEvent,
...
AutoRef::AutoRef() {
  // ctor
  AutoRefqueue = xQueueCreate(QUEUE_DEPTH_AUTOREF, sizeof(uint32_t));
}
...
void AutoRef::begin() {
  if (m_HasBegun)
    return;
  xTaskCreatePinnedToCore(AutoRefHandler,   /* Task function. */
                          "AutoRefHandler", /* String with name of task. */
                          STACK_AUTOREF,    /* Stack size in bytes. */
                          NULL, /* Parameter passed as input of the task */
                          PRIORITY_AUTOREF, /* Priority of the task. */

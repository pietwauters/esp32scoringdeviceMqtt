...
enum TimerStatus_t {
  NO_TIME,       // No need to show the time, it's not available
...
public:
  ...
  void DisplayTime(uint8_t minutes, uint8_t seconds, uint8_t hundreths,
                   bool TenthsOnly = true);
  void SetScore(uint8_t scoreLeft, uint8_t scoreRight) {
    m_scoreLeft = scoreLeft;
    m_scoreRight = scoreRight;
  };
  void SetTime(uint8_t minutes, uint8_t seconds, uint8_t hundreths) {
    m_minutes = minutes;
    m_seconds = seconds;
    m_hundredths = hundreths;
  };
  ...
  void ShowTime() {
    DisplayTime(m_minutes, m_seconds, m_hundredths);
  ...
  void DisplayPisteId();
  void DisplayResetReason(int reason);
  void DisplayVersion();
  void LaunchStartupDisplay();
  void SetBrightness(int value);
  static void SetPower(bool bValue) {
    if (bValue) {

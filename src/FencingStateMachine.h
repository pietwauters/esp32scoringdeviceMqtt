...
class UDPIOHandler;
class CyranoHandler;
class FencingStateMachine : public Subject<FencingStateMachine>,
                            public Observer<MultiWeaponSensor>,
                            public Observer<UDPIOHandler>,
                            public Observer<CyranoHandler>,
                            public SingletonMixin<FencingStateMachine> {
public:
  /** Default destructor */
   ...
   * \param val New value to set
   */
  void SetPriority(Priority_t val) {
    if (val != m_Priority) {
      m_Priority = val;
      m_StateChanged = true;
    }
   ...
   * \param val New value to set
   */
  void SetYellowCardLeft(int val) {
    if (val != m_YellowCardLeft) {
      m_YellowCardLeft = val;
      m_StateChanged = true;
    }
  }
   ...
   * \param val New value to set
   */
  void SetYellowCardRight(int val) {
    if (val != m_YellowCardRight) {
      m_YellowCardRight = val;
      m_StateChanged = true;
    }
  }
   ...
   * \param val New value to set
   */
  void SetRedCardLeft(int val) {
    if (val != m_RedCardLeft) {
      m_RedCardLeft = val;
      m_StateChanged = true;
    }
  }
   ...
   * \param val New value to set
   */
  void SetRedCardRight(int val) {
    if (val != m_RedCardRight) {
      m_RedCardRight = val;
      m_StateChanged = true;
    }
  }
   ...
   * \param val New value to set
   */
  void SetTimerstate(TimerState_t val) {
    if (val != m_Timerstate) {
      m_Timerstate = val;
      m_StateChanged = true;
    }
   ...
   * \param val New value to set
   */
  void SetScoreLeft(unsigned int val) {
    if (val != m_ScoreLeft) {
      m_ScoreLeft = val;
      m_StateChanged = true;
    }
   ...
   * \param val New value to set
   */
  void SetScoreRight(unsigned int val) {
    if (val != m_ScoreRight) {
      m_ScoreRight = val;
      m_StateChanged = true;
    }
   ...
   * \param val New value to set
   */
  void SetMachineWeapon(weapon_t val) {
    if (val != m_MachineWeapon) {
      m_MachineWeapon = val;
      m_WeaponChanged = true;
    }
  }
  void SetMachineLights(uint32_t val) {
    if (val != m_Lights) {
      m_Lights = val;
      m_LightsChanged = true;
    }
  }
  void RegisterMultiWeaponSensor(MultiWeaponSensor *MySensor) {
    m_TheSensor = MySensor;
  ...
  void GetFormattedStringTime(char *Destination, int MinutePrecision,
    ...
    m_Timer.GetFormattedStringTime(Destination, MinutePrecision,
                                   HundredthsPrecision);
  };
...
private:
  ...
  FencingStateMachine(int hw_timer_nr = 2,
                      int tickPeriod = 10); // tickPeriod in miliseconds
